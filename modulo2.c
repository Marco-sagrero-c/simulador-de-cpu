#include "swap.h"
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <ncurses.h>
#include <sys/select.h>
#include <time.h>
#include "kbhit.h"
#include <ctype.h>
#include <limits.h>
#include "lista.h"

#define QMAX 10

void draw(struct PCB *pcb, int marcos);
void strUpper(char *str);
int procesar_instruccion(char *instruccion, struct PCB *pcb);
// int leer_linea_archivo(FILE *archivo, struct PCB *pcb, struct PCB **lista);
int comandos(char *mensaje, struct PCB **lista, struct PCB **pcb, struct PCB **, struct PCB **);
int shell(char *mensaje, int *j);
//////////////////////////////////
struct PCB *crear_pcb();
void ver_lista(struct PCB *lista);
// void encolar_pcb(struct PCB **lista, struct PCB **pcb);
int programa_cuenta();
// MODULO 2
void ver_terminados(struct PCB *lista);
void mensage(char *mensaje);
void ver_principal(struct PCB *nodo);
int tecla_especial(int *j, int *ch, char *mensaje, struct PCB **lista, struct PCB **PRINCIPAL, struct PCB **terminados, int *rapides, int *TMP, int long *swap, struct PCB **LISTOS, int *marcos);
// MODULO 3
int usuario(struct PCB *lista, struct PCB *Prim);
void Dispacher(struct PCB **lista1);
void actu_U(struct PCB **lista, int us);
void Actualiza_p(struct PCB **lista);
int el_menor(struct PCB *lista);

///////
int IncCPU = 60 / QMAX; // Quantum por proceso
int PBase = 60;         // Prioridad base para todos los procesos
int NumUs = 0;          // Cantidad de usuarios para planificar
int W = 0;
// Peso o ponderación de usuarios (inicializado en 0.0)
int usu[100]; // Arreglo con IDs de usuarios, suponiendo un máximo de 100 usuarios

////////////////////modulo 4
void VER_TMS(int Cantidad);
void ver_swap(int long posicion);
void ver_listos(struct PCB *lista);
void liberarMarcosPorPID(int pid);
int obtenerMarco(int pid, int arco);
int leer_linea_archivoBin(struct PCB *pcb, struct PCB **lista);
int buscarProgramaPorNombre(struct PCB *lista, struct PCB *PCBejecucion, struct PCB *PCbnuevo);
int hijo(struct PCB *lista, int padre);
void heredar(int pid, int sobrino, struct PCB **lista, struct PCB **PCB);
/////esta parte del codigo es para la RAM

char Ram[64][32] = {0};

void ver_ram();
void mostrar_cap_ram();

int manejar_error_pagina(int pid, int pagina, struct ram *ram, struct tmp *swap);
// Función para seleccionar un marco para reemplazo (algoritmo LRU)
int seleccionar_marco_reemplazo(struct ram *ram);
// Función para guardar una página en el swap
void guardar_en_swap(int pid, int pagina, struct tmp *swap);
// Función para cargar una página desde el swap a la RAM
void cargar_desde_swap(int pid, int pagina, struct ram *marco, struct tmp *swap);
// struct ram RAM= NULL;
/////
int verificar_fallo_pagina(int pid, int pagina);
int copiar_marco_completo_swap_a_ram(int pid, int marco_swap, long linea);
int planificador_fifo();
int planificador_reloj();
int puntero_reloj;
int tiempo_global_lru = 0;
char *leer_linea_cadenaXD(long int posicion);
int ver_ram_con = 0;

int planificador_fifo();
void mostrar_cap_ram_fifo();
int planificador_lru();
void actualizar_uso_si_toca(int marco_actual);
void mostrar_cap_ram_lru();
////car

int algoridmo_CAR();
void mostrar_estado_car();
int p = 0; // variable p

typedef struct
{
    int id;
    int num_marco;
} ListaFantasma;

ListaFantasma B1[TAMANO_B1_B2];
ListaFantasma B2[TAMANO_B1_B2];

int tam_b1 = 0, tam_b2 = 0;
int puntero_clock_t1 = 0;
int puntero_clock_t2 = 0;

void agregar_a_B1(int id, int marco);
void agregar_a_B2(int id, int marco);

int verificar_historial(int id, int marco);
void eliminar_de_B2(int id, int num_marco);
void eliminar_de_B1(int id, int num_marco);

int contar_T1();
int meter_b1_b2(int pid, int i);
char *leer_linea_ram(int fila);
int obtener_marco_en_ram(int pid, int numero_logico);
///

int crear_proceso_con_archivo(char *b, struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS);
void crea_multiples_p(struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS);

void guardar_datos(int a, int b);

int A_Replazo_pagina = 0; // 0 reloj , 1 fifo , 2 url ,3 car

void (*mostrar_funciones[])() = {
    mostrar_cap_ram,      // índice 0
    mostrar_cap_ram_fifo, // índice 1
    mostrar_cap_ram_lru,
    mostrar_estado_car // índice 2
};

int (*A_Remplazo_P[])() = {
    planificador_reloj, // índice 0
    planificador_fifo,  // índice 1
    planificador_lru,
    algoridmo_CAR // índice 2
};

//////////////////////fallos de pagina
int fallos_de_pagina;
int contar_fallos;
// int accesos_a_memoria;

int main(void)
{

    ///////////////archivo binario (swap)
    char archivoSalida[] = "SWAP.bin"; // Nombre del archivo binario de salida
    salida = fopen(archivoSalida, "wb+");
    if (salida == NULL)
    {
        perror("Error al crear archivo binario de salida");
        return EXIT_FAILURE;
    }

    for (int i = 0; i < TAMANO_ARREGLO; i++)
    {
        long x = i * 4;
        arreglo[i].lleno = 0;
        arreglo[i].pri = x;
        arreglo[i].ult = x + 4;
        arreglo[i].tmp = i;
    }

    for (int i = 0; i < TAMANO_RAM; i++)
    {
        int x = i * 4;        // Cada marco ocupa 512 bytes
        ram_arr[i].pri = x;   // Dirección de inicio del marco en RAM
        ram_arr[i].lleno = 0; // Indicar que el marco está vacío
        ram_arr[i].id = -1;   // No tiene proceso asignado
    }


    int TMP = 0;
    int long swap = 0;
    int conta = 0;

    struct PCB *PRINCIPAL = crear_PCB(-1); // este pcb es el que se va a ejecutar en el prosesador -1 VACIO !1 TIENE ALGO
    PRINCIPAL->IR = "--------";
    PRINCIPAL->fileName = "VACIO     ";
    PRINCIPAL->programa = 0;
    struct PCB *lista = NULL;      // ESTA ES LA LISTA DE ESPERA
    struct PCB *AUX = NULL;        // ESTA LISTA ES PARA PODER HACER VARIOS TRABAJOS A LA VEZ
    int qmax = 0;                  // esta es la variable quue controle el q max
    struct PCB *terminados = NULL; // ESTA ES LA LISTA DE LOS QUE YA SE TERMINARON
    struct PCB *LISTOS = NULL;     // ESTA ES LA LISTA DE LOS QUE YA SE TERMINARON
    struct PCB *AUXL = NULL;       // ESTA LISTA ES PARA PODER HACER VARIOS TRABAJOS A LA VEZ
    int rapidez = 10;
    int iteraciones = 0;
    int ch;

    int control = 1;
    char mensaje[80] = {0};
    char copia[80];
    int j = 0;
    //////////////////////////////
    int marcos = 0;

    initscr(); // Inicializa ncurses
    resize_term(300, 300);
    raw();                // Desactiva el búfer de línea
    keypad(stdscr, TRUE); // Habilita la captura de teclas especiales
    noecho();             // No muestra los caracteres ingresados
    strcpy(copia, "esto es un teexto de prueba");
    while (control)
    {
        ch = shell(mensaje, &j);
        iteraciones++;

        control = tecla_especial(&j, &ch, mensaje, &lista, &AUX, &terminados, &rapidez, &TMP, &swap, &LISTOS, &marcos);

        if (strcmp(copia, mensaje))
        {
            mensage(mensaje);
            strcpy(copia, mensaje);
            refresh();
        }

        if (iteraciones == rapidez)
        {
            iteraciones = 0;

            if (lista != NULL)
            {
                if (AUX == NULL)
                {
                    // dispacher
                    int menor = el_menor(lista);
                    mvprintw(11, 0, "menor = %d", menor);
                    insertar_PCB(&AUX, extraer_PCB(&lista, menor));
                }

                if (lista != NULL)
                {
                    ver_lista(lista);
                }
                else
                {
                    mvprintw(4, 70, "                                                                                                                                                                                           ");
                    refresh();
                }
            }
            else
            {
                mvprintw(1, 70, "                                                                                                                                                                                ");
                draw(PRINCIPAL, 0);
            }

            if (AUX != NULL)
            {
                if (qmax == IncCPU)
                {
                    insertar_PCB(&lista, extraer_PCB(&AUX, AUX->pid));
                    AUX = NULL;
                    qmax = 0;
                    Dispacher(&lista);

                    // DISPACHER
                }
                else
                {

                    if (leer_linea_archivoBin(AUX, &lista) == 1)
                    {
                        draw(AUX, marcos);
                        ver_principal(AUX);
                    }
                    else
                    {

                        int familia = hijo(lista, AUX->pid);
                        if (familia == 0)
                        {
                            liberarMarcosPorPID(AUX->pid);
                            // agregar a b1 o b2
                        }
                        else
                        {
                            heredar(AUX->pid, familia, &lista, &AUX);
                        }

                        insertar_PCB(&terminados, extraer_PCB(&AUX, AUX->pid));
                        AUX = NULL;
                        Dispacher(&lista);
                        /////DISPACHER
                    }
                }

                qmax++;
            }
            if (LISTOS != NULL)
            {
                ver_listos(LISTOS);
                AUXL = extraer_PCB(&LISTOS, LISTOS->pid);

                int familia = buscarProgramaPorNombre(lista, AUX, AUXL);

                if (familia != 0)
                {
                    AUXL->familia = familia;
                    insertar_PCB(&lista, AUXL);
                    Dispacher(&lista);
                }
                else
                {
                    conta = bloques(AUXL->programa);
                    if (conta <= espacios_restantes())
                    {
                        fseek(AUXL->programa, 0, SEEK_SET);
                        escribirParteEnArchivoBinario(AUXL->programa, AUXL->pid);
                        insertar_PCB(&lista, AUXL);
                        Dispacher(&lista);
                    }
                    else
                    {
                        insertar_PCB(&LISTOS, AUXL);
                    }
                }
            }
            else
            {
                mvprintw(16, 70, "                                                                                                                  ");
                mvprintw(17, 70, "                                                                                                                  ");
                refresh();
            }
        }

        if (terminados != NULL)
        {
            ver_terminados(terminados);

            
        }

        if (iteraciones == 200)
        {
            iteraciones = 0;
        }
        
        ver_ram();

        mostrar_funciones[A_Replazo_pagina]();
        // mostrar_estado_car();

        refresh();

        usleep(1000); // DILEI
    }

    endwin(); // Termina ncurses
    PRINCIPAL->fileName = NULL;
    free(PRINCIPAL->fileName);
    PRINCIPAL->IR = NULL;
    free(PRINCIPAL->IR);
    free(PRINCIPAL);
    vaciar_lista(&lista);

    vaciar_lista_terminados(&terminados);

    fclose(salida);

    guardar_datos(fallos_de_pagina, accesos_a_memoria);

    return EXIT_SUCCESS;
}

void liberarMarcosPorPID(int pid)
{

    for (int i = 0; i < TAMANO_ARREGLO; i++)
    {
        if (arreglo[i].id == pid)
        {
            arreglo[i].lleno = 0; // Marcar como vacío
            arreglo[i].id = 0;    // Reiniciar PID
            meter_b1_b2(pid, i);  // los meto en b1 o en b2
        }
    }

    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].id == pid)
        {
            ram_arr[i].lleno = 0;
            ram_arr[i].id = 0;
        }
    }
}

void heredar(int pid, int sobrino, struct PCB **lista, struct PCB **PCB)
{
    for (int i = 0; i < TAMANO_ARREGLO; i++)
    {
        if (arreglo[i].id == pid)
        {
            arreglo[i].id = sobrino;
        }
    }

    struct PCB *actual = *lista;

    while (actual != NULL)
    {
        if (actual->familia == pid)
        {
            actual->familia = sobrino;
        }
        actual = actual->sig;
    }

    //////
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (pid == ram_arr[i].id)
        {
            ram_arr[i].id = sobrino;
        }
    }

    // ram_arr[marco_ram].id = pid;
}

int hijo(struct PCB *lista, int padre)
{

    while (lista != NULL)
    {
        if (lista->familia == padre)
        {
            return lista->pid;
        }
        lista = lista->sig;
    }
    return 0;
}

int obtenerMarco(int pid, int Marco)
{
    int count = 0;
    for (int i = 0; i < TAMANO_ARREGLO; i++)
    {
        if (arreglo[i].id == pid)
        {
            if (count == Marco)
            {
                return i; // Devuelve el índice del marco correspondiente
            }
            count++;
        }
    }
    return -1; // Si no se encuentra ningún marco para el pid y Marco dados, devuelve -1
}

int el_menor(struct PCB *lista)
{
    // Inicializamos el valor mínimo con el máximo posible
    int minP = INT_MAX;
    int uidmin = -1;

    // Recorremos la lista
    while (lista != NULL)
    {
        // Si el P actual es menor que el mínimo encontrado hasta ahora, actualizamos minP
        if (lista->P < minP)
        {
            minP = lista->P;
            uidmin = lista->pid;
        }
        // Pasamos al siguiente nodo
        lista = lista->sig;
    }
    // Devolvemos el valor mínimo de P
    // MIN P ES EL MINIMO
    return uidmin;
}

void Dispacher(struct PCB **lista1)
{
    struct PCB *AUX1 = *lista1;
    int a = usuario(*lista1, NULL);
    if (a <= 0)
    {
        a = 1;
    }
    W = 100 / a;
    Actualiza_p(lista1);
}

void Actualiza_p(struct PCB **lista)
{
    struct PCB *actual = *lista;
    bool actualizado[100] = {false}; // NUM_USUARIOS es el número total de usuarios posible

    while (actual != NULL)
    {
        actual->KCPU = actual->KCPU / 2;

        if (!actualizado[actual->UID]) // Verifica si el usuario no ha sido actualizado
        {
            usu[actual->UID] = usu[actual->UID] / 2;
            actualizado[actual->UID] = true; // Marca al usuario como actualizado
        }
        actual->KCPUxU = usu[actual->UID];
        actual->P = PBase + (actual->KCPU / 2) + (usu[actual->UID] / (4 * W * 0.01));
        actual = actual->sig;
    }
}

int usuario(struct PCB *lista1, struct PCB *lista2)
{
    int contador = 0;
    int *uidUnicos = NULL; // Arreglo para almacenar los UID únicos
    struct PCB *actual = lista1;

    // Recorrer la primera lista
    while (actual != NULL)
    {
        // Verificar si el UID actual ya está en la lista de UID únicos
        int encontrado = 0;
        for (int i = 0; i < contador; i++)
        {
            if (uidUnicos[i] == actual->UID)
            {
                encontrado = 1;
                break;
            }
        }

        // Si el UID no está en la lista de UID únicos, agregarlo
        if (!encontrado)
        {
            contador++;
            uidUnicos = realloc(uidUnicos, contador * sizeof(int));
            uidUnicos[contador - 1] = actual->UID;
        }

        actual = actual->sig; // Avanzar al siguiente PCB en la lista
    }

    // Recorrer la segunda lista
    actual = lista2;
    while (actual != NULL)
    {
        // Verificar si el UID actual ya está en la lista de UID únicos
        int encontrado = 0;
        for (int i = 0; i < contador; i++)
        {
            if (uidUnicos[i] == actual->UID)
            {
                encontrado = 1;
                break;
            }
        }

        // Si el UID no está en la lista de UID únicos, agregarlo
        if (!encontrado)
        {
            contador++;
            uidUnicos = realloc(uidUnicos, contador * sizeof(int));
            uidUnicos[contador - 1] = actual->UID;
        }

        actual = actual->sig; // Avanzar al siguiente PCB en la lista
    }

    free(uidUnicos); // Liberar la memoria del arreglo de UID únicos
    return contador;
}

int tecla_especial(int *j, int *ch, char *mensaje, struct PCB **lista, struct PCB **PRINCIPAL, struct PCB **terminados, int *rapides, int *TMP, int long *swap, struct PCB **LISTOS, int *marcos)
{

    if (*ch == 10) // SIRVE PARA LEEER COMANDOS
    {
        if (comandos(mensaje, lista, PRINCIPAL, terminados, LISTOS) == -935)
            return 0;
        mensaje[0] = '\0';
        *j = 0;
        mvprintw(14, 2, "                                                                                          ");
        refresh();
    }

    if (*ch == 27) // DETECTA SI LA TECLA PRESIONADA ES ESC
    {
        return 0;
    }

    if (*ch == KEY_BACKSPACE)
    {

        if ((*j) > 0)
        {
            (*j)--;
            (*j)--;             // Retroceder en el índice del mensaje
            mensaje[*j] = '\0'; // Eliminar el último carácter del mensaje
        }
    }

    if (*ch == KEY_LEFT) // IZQUIERDA
    {
        if (*rapides > 10)
        {
            (*rapides) -= 10;
        }

        (*j)--; // Retroceder en el índice del mensaje
        mensaje[*j] = '\0';
        mvprintw(13, 2, "                                                      ");
        mvprintw(13, 2, "rapidez %d                        ", *rapides);
        refresh();
    }
    if (*ch == KEY_RIGHT) // DERECHA
    {
        if (*rapides < 210)
        {
            (*rapides) += 10;
        }

        (*j)--;
        mensaje[*j] = '\0';
        mvprintw(13, 2, "                                                         ");
        mvprintw(13, 2, "rapidez %d                       ", *rapides);
        refresh();
    }

    if (*ch == KEY_UP) // DERECHA
    {
        if (*TMP < 4080)
        {
            (*TMP) += 16;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    if (*ch == KEY_DOWN) // DERECHA
    {
        if (*TMP > 0)
        {
            (*TMP) -= 16;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    if (*ch == KEY_F(2)) // DERECHA
    {
        if (*swap <= 65456)
        {
            (*swap) += 80;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    ////

    if (*ch == KEY_F(4)) // DERECHA
    {
        if (*marcos <= 4095)
        {
            (*marcos) += 1;
        }
        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    if (*ch == KEY_F(3)) // DERECHA
    {
        if (*marcos > 0)
        {
            (*marcos) -= 1;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    if (*ch == KEY_F(6)) // DERECHA
    {
        if (ver_ram_con > 0)
        {
            ver_ram_con -= 1;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }

    if (*ch == KEY_F(7)) // DERECHA
    {
        if (ver_ram_con < (TAMANO_RAM / 4) - 1)
        {
            ver_ram_con += 1;
        }

        (*j)--;
        mensaje[*j] = '\0';
        refresh();
    }
    ///

    if (*ch == KEY_F(1)) // DERECHA
    {
        if (*swap > 0)
        {
            (*swap) -= 80;
        }

        (*j)--;
        mensaje[*j] = '\0';
        mvprintw(13, 2, "                                                         ");
        mvprintw(13, 2, "rapidez %d                       ", *rapides);
        refresh();
    }

    return 1;
}

void ver_principal(struct PCB *nodo)
{
    struct PCB *actual = nodo;
    mvprintw(0, 70, "-------------------------ejecucion----------------------------------------------------------------------------------------------");
    // mvprintw(1, 70, "PID:[   ] FILE:[                ] AX:[   ] BX:[   ] CX:[   ] DX:[   ] PC:[   ] IR:[        ] UID[  ] KCPU[   ] KCPUxU[   ] P[  ]");
    mvprintw(1, 70, "PID:[ %d  ] FILE:[ %s ] AX:[ %d ] BX:[ %d ] CX:[ %d ] DX:[ %d ] PC:[ %d ] IR:[ %s ]", actual->pid, actual->fileName, actual->AX, actual->BX, actual->CX, actual->DX, actual->PC-1, actual->IR);
    //mvprintw(1 + 1, 70, "                                                                                                ");
    mvprintw(0, 2, "$> ");
    // mvprintw(1, 167, "%d", actual->UID);
    // mvprintw(1, 176, "%d", actual->KCPU);
    // mvprintw(1, 188, "%d", actual->KCPUxU)
    mvprintw(1 + 1, 70, "                                                                                                                                                                                                                 ");
    refresh();
}

void ver_lista(struct PCB *lista)
{

    struct PCB *actual = lista;
    int contador = 3;
    mvprintw(3, 70, "-------------------------LISTOS-------------------------------------------------------------------------");
    while (actual != NULL)
    {
        contador++;
        // mvprintw(contador, 70, "PID:[   ] FILE:[                ] AX:[   ] BX:[   ] CX:[   ] DX:[   ] PC:[   ] IR[         ] UID[  ] KCPU[   ] KCPUxU[   ] P[  ]");
        mvprintw(contador, 70, "PID:[ %d  ] FILE:[ %s ] AX:[ %d ] BX:[ %d ] CX:[ %d ] DX:[ %d ] PC:[ %d ] IR:[ %s ]", actual->pid, actual->fileName, actual->AX, actual->BX, actual->CX, actual->DX, actual->PC-1, actual->IR);
        mvprintw(contador + 1, 70, "                                                                                                ");
        mvprintw(0, 2, "$> ");
        actual = actual->sig;


    }
    refresh();
}

void ver_listos(struct PCB *lista)
{

    struct PCB *actual = lista;
    int contador = 16;
    mvprintw(16, 70, "-------------------------ESPERA-------------------------------------------------------------------------");
    while (actual != NULL)
    {
        contador++;
        mvprintw(contador, 70, "PID:[ %d  ] FILE:[ %s ] AX:[ %d ] BX:[ %d ] CX:[ %d ] DX:[ %d ] PC:[ %d ] IR:[ %s ]", actual->pid, actual->fileName, actual->AX, actual->BX, actual->CX, actual->DX, actual->PC-1, actual->IR);
        mvprintw(contador + 1, 70, "                                                                                                                                    ");
        mvprintw(0, 2, "$> ");
        actual = actual->sig;
    }
    refresh();
}

void VER_TMS(int cuales)
{

    int contador = cuales + 15;
    int aux = 38;
    mvprintw(37, 0, "-------TMS----------");

    while (cuales <= contador)
    {

        mvprintw(aux, 0, "MARCO:%x  PID:%d LLENO:%d ", cuales, arreglo[cuales].id, arreglo[cuales].lleno);
        // mvprintw(aux + 1, 0, "                                                                      ");
        mvprintw(0, 2, "$> ");
        cuales++;
        aux++;
    }
    refresh();
}

void ver_terminados(struct PCB *lista)
{
    struct PCB *actual = lista;
    int contador = 25;
    mvprintw(25, 70, "---------------------------------------------------terminados-----------------");
    while (actual != NULL)
    {
        if (contador < 35)
        {
            contador++;
        }
        mvprintw(contador, 70, "PID:[ %d  ] FILE:[ %s ] AX:[ %d ] BX:[ %d ] CX:[ %d ] DX:[ %d ] PC:[ %d ] IR:[ %s ]", actual->pid, actual->fileName, actual->AX, actual->BX, actual->CX, actual->DX, actual->PC-1, actual->IR);
        mvprintw(contador + 1, 70, "                                                                                                ");
        mvprintw(0, 2, "$> ");
        //(mvprintw(25,70,"el pid final %d", actual->pid);
        actual = actual->sig;
    }

    refresh();
}


struct PCB *crear_pcb() // dios 1 si crea dios 0 solo cuenta
{
    int a = programa_cuenta();
    return crear_PCB(a);
}

int programa_cuenta()
{
    static int contador = 0; // Se inicializa en la primera llamada y retiene su valor entre llamadas
    contador++;
    return contador;
}

void draw(struct PCB *pcb, int marcos)
{

    mvprintw(1, 0, "---------------------------procesador-------------------------");
    mvprintw(2, 0, "                      ");
    mvprintw(2, 0, "NAME:%s", pcb->fileName);
    mvprintw(3, 0, "           ");
    mvprintw(3, 0, "-AX:[%d]", pcb->AX);
    mvprintw(3, 10, "           ");
    mvprintw(3, 10, "-PC:[%d]", pcb->PC - 1); //
    mvprintw(4, 0, "          ");
    mvprintw(4, 0, "-BX:[%d]", pcb->BX);
    mvprintw(5, 0, "          ");
    mvprintw(5, 0, "-CX:[%d]", pcb->CX);
    mvprintw(6, 0, "          ");
    mvprintw(6, 0, "-DX:[%d]", pcb->DX);
    mvprintw(4, 10, "          ");
    mvprintw(4, 10, "-PID:[%d] ", pcb->pid);
    mvprintw(9, 0, "              ");
    mvprintw(9, 0, "IR:[%s]", pcb->IR);
    mvprintw(5, 10, "           ");
    // mvprintw(5, 10, "-UDI:[%d]", pcb->UID);
    mvprintw(6, 10, "          ");
    // mvprintw(6, 10, "-KCPU:[%d] ", pcb->KCPU);
    mvprintw(7, 0, "          ");
    // mvprintw(7, 0, "-KCPUxU:[%d]", pcb->KCPUxU);
    mvprintw(8, 0, "          ");
    mvprintw(8, 0, "-P:[%d]", pcb->P);
    mvprintw(12, 0, "-------------MENSAJES-------");
    mvprintw(14, 0, "-");
    mvprintw(15, 0, "-");
    mvprintw(18, 0, "                         ");
    mvprintw(18, 0, "%d", fallos_de_pagina);
    mvprintw(0, 2, "$>");
    /////////////////////////////////////////////////////////TMS
    /*
    if (pcb->pid == -1)
    {
        mvprintw(8, 10, "-PAG:[0]");
        for (int i = 2; i < 12; i++)
        {
            mvprintw(i, 23, "                                       ");
        }
    }
    else
    {
        mvprintw(8, 10, "-PAG:[%d]", bloques(pcb->programa));

        mvprintw(2, 23, "-MR--------------TMP--------MV----------");

        for (int i = 3; i < 12; i++)
        {
            if (marcos < bloques(pcb->programa))
            {

                if (pcb->familia != 0)
                {
                    mvprintw(i, 40, "-MR %x     MV %x", obtenerMarco(pcb->familia, marcos), arreglo[marcos].orden);
                }
                else
                {
                    mvprintw(i, 40, "-MR %x     MV %x", obtenerMarco(pcb->pid, marcos), arreglo[marcos].orden);
                }
            }
            else
            {
                mvprintw(i, 40, "-MR %d   MV 0 ", arreglo[marcos].tmp);
            }
            mvprintw(i + 1, 40, "                        ");
            marcos++;
        }

    }
    */
    refresh(); // Actualiza la pantalla
}

void mensage(char *mensaje)
{
    mvprintw(0, 2, "$>                                                  ");
    mvprintw(0, 2, "$> %s", mensaje);
    refresh(); // Actualiza la pantalla
}

void strUpper(char *str)
{
    while (*str)
    {
        *str = toupper(*str);
        str++;
    }
}

void actu_U(struct PCB **lista, int us)
{

    struct PCB *actual = *lista;
    while (actual != NULL)
    {
        if (actual->UID == us)
        {
            actual->KCPUxU = usu[us];
        }

        actual = actual->sig;
    }
}

int buscarProgramaPorNombre(struct PCB *lista, struct PCB *PCBejecucion, struct PCB *PCbnuevo)
{
    struct PCB *actual = lista;

    if (PCBejecucion != NULL)
    {

        if (strcmp(PCbnuevo->fileName, PCBejecucion->fileName) == 0 && PCbnuevo->UID == PCBejecucion->UID)
        {
            return PCBejecucion->pid;
        }
    }

    // Recorrer la lista de PCBs
    while (actual != NULL)
    {
        if (strcmp(actual->fileName, PCbnuevo->fileName) == 0 && PCbnuevo->UID == actual->UID)
        {
            return actual->pid; // Retornar el pid si se encuentra coincidencia
        }
        actual = actual->sig; // Avanzar al siguiente nodo
    }

    return 0; // Retornar -1 si no se encuentra coincidencia
}

int comandos(char *mensaje, struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS)
{
    char a[80] = {0}, b[80] = {0}, c[5] = {0};
    // sscanf(mensaje, "%s %s %s", a, b, c);
    sscanf(mensaje, "%s %s ", a, b);
    FILE *archivo = NULL; // Archivo abierto

    strUpper(a);

    if (strcmp(a, "LOAD") == 0)
    {
        // if ((strcmp(b, "\n") == 0) || !(isdigit(c[0])))
        if ((strcmp(b, "\n") == 0))
        {
            mvprintw(13, 2, "                                                            ");
            mvprintw(13, 2, "FORMATO INCORRECTO LOAD archivo   ");
            refresh();
            return 0;
        }
        // int UID = atoi(c);
        int UID = 1;
        mvprintw(13, 2, "                                                         ");
        mvprintw(13, 2, "se selecciono el archivo %s con usuario %d", b, UID);
        refresh();

        if (archivo = fopen(b, "r"))
        {
            struct PCB *PCB = crear_pcb();
            strcpy(PCB->fileName, b);
            strcpy(PCB->IR, "----------");
            PCB->programa = archivo;
            PCB->UID = UID;
            PCB->P = PBase;              ///
            PCB->KCPUxU = usu[PCB->UID]; // hacer el pcb nuevo
            PCB->marcos = bloques(archivo);

            int parentesco = buscarProgramaPorNombre(*lista, *pcb, PCB);
            int resultado = 1;

            // if (parentesco > 0)
            if (false)
            {
                // printf("hay familia  %d ------%d ...",parentesco,PCB->pid);
                PCB->familia = parentesco;
            }
            else
            {
                resultado = escribirParteEnArchivoBinario(PCB->programa, PCB->pid);
            }

            if (resultado == 0)
            {
                insertar_PCB(terminados, PCB);
                mvprintw(13, 2, "                                                ");
                mvprintw(13, 2, "archivo es muy grande %s ", b);
                refresh();
                return 0;
            }
            else if (resultado == 41)
            {
                insertar_PCB(LISTOS, PCB);
                return 1;
            }
            insertar_PCB(lista, PCB);
            Dispacher(lista); //
        }
        else
        {
            mvprintw(13, 2, "                                                ");
            mvprintw(13, 2, "archivo no encontrado %s ", b);
            refresh();
            return 0;
        }

        return 1;
    }
    else if (strcmp(a, "PRUEBA") == 0)
    {
        crea_multiples_p(lista, pcb, terminados, LISTOS); 
    }
    else if (strcmp(a, "CLEAR") == 0)
    {
        fallos_de_pagina = 0;
        vaciar_lista_terminados(terminados);
        accesos_a_memoria = 0;
    }
    else if (strcmp(a, "CAP") == 0)
    {
        vaciar_lista_terminados(terminados);
        guardar_datos(fallos_de_pagina, accesos_a_memoria);
        fallos_de_pagina = 0;
        accesos_a_memoria = 0;
    }
    else if (strcmp(a, "CAMBIO") == 0)
    {
        if ((strcmp(b, "\n") == 0))
        {
            mvprintw(13, 2, "                                                            ");
            mvprintw(13, 2, "FORMATO INCORRECTO LOAD archivo UsuarioID  ");
            refresh();
            return 0;
        }
        int algridmo = atoi(b);

        if (algridmo >= 0 && algridmo < 4)
        {
            A_Replazo_pagina = algridmo;
        }
        else
        {
            mvprintw(13, 2, "                                                            ");
            mvprintw(13, 2, "algoridmo no valido 0->reloj 1-> fifo 2->lru");
            refresh();
            return 0;
        }
    }
    else if (strcmp(a, "END") == 0)
    {
        mvprintw(13, 2, "                                                                ");
        mvprintw(13, 2, "saliendo........ ");
        refresh();
        getch();
        return -935;
    }
    else if (strcmp(a, "KILL") == 0)
    {
        int auxpid = 0;
        if (isdigit(b[0]))
        {
            auxpid = atoi(b);
        }
        else
        {
            mvprintw(13, 2, "                                                       ");
            mvprintw(13, 2, "FORMATO-> KILL <PID>               ");
            refresh();
            return 0;
        }

        if ((*pcb) == NULL)
        {
            mvprintw(13, 2, "                                                             ");
            mvprintw(13, 2, "no se encontro el pid N             ");
            refresh();
            return 0;
        }

        if ((*pcb)->pid == auxpid)
        {
            struct PCB *eliminar = *pcb;

            int familia = hijo(*lista, eliminar->pid);

            if (familia == 0)
            {
                liberarMarcosPorPID(eliminar->pid);
                // agregar a b1 o b2
            }
            else
            {
                heredar(eliminar->pid, familia, lista, pcb);
            }

            insertar_PCB(terminados, eliminar);
            *pcb = NULL;
            Dispacher(lista); //
        }
        else
        {
            if (*lista == NULL)
            {
                mvprintw(13, 2, "                                                             ");
                mvprintw(13, 2, "lista vacia                        ");
                refresh();
                return 0;
            }

            struct PCB *eliminar = extraer_PCB(lista, auxpid);
            if (eliminar == NULL)
            {
                mvprintw(13, 2, "                                                                    ");
                mvprintw(13, 2, "no se encontro el pid              ");
            }
            else
            {
                ver_lista(*lista);

                int familia = hijo(*lista, eliminar->pid);

                if ((*pcb)->familia == eliminar->pid)
                {
                    heredar(eliminar->pid, (*pcb)->pid, lista, pcb);
                }
                else if (familia == 0)
                {
                    liberarMarcosPorPID(eliminar->pid);
                    // agregar a b1 o b2
                }
                else
                {
                    heredar(eliminar->pid, familia, lista, pcb);
                }

                insertar_PCB(terminados, eliminar);
                Dispacher(lista);
            }
        }
    }
    else
    {
        mvprintw(13, 2, "                                                                                ");
        mvprintw(13, 2, "opcion no valida ");
        refresh();
        return 1;
    }
}

int shell(char *mensaje, int *j)
{
    int ch = 0;
    if (kbhit())
    {
        do
        {
            ch = getch(); // Obtener la tecla presionada
        } while (kbhit());
        mensaje[(*j)++] = ch;
        int tr = *j;
        if (tr > 79)
        { // no deja que el mensage supere los 80 caraterers no tiene mensage
            mensaje[0] = '\0';
            *j = 0;
        }
        mensaje[tr] = '\0';
    }
    return ch;
}

void mostrar_mensaje(const char *mensaje)
{
    mvprintw(14, 2, "                                                      ");
    mvprintw(14, 2, "%s", mensaje);
    mvprintw(0, 2, "$>");
    refresh();
}

// Función para obtener el valor de un registro o número
int obtener_valor(const char *registro_fuente, struct PCB *pcb)
{
    if (isdigit(registro_fuente[0]))
    {
        return atoi(registro_fuente);
    }

    // Arreglo de punteros a los registros para simplificar el acceso
    int *registros[] = {&pcb->AX, &pcb->BX, &pcb->CX, &pcb->DX};
    const char *nombres_registros[] = {"AX", "BX", "CX", "DX"};

    for (int i = 0; i < 4; i++)
    {
        if (strcmp(registro_fuente, nombres_registros[i]) == 0)
        {
            return *registros[i];
        }
    }

    mostrar_mensaje("registro no valido");
    return -1; // Valor inválido
}

int procesar_instruccion(char *instruccion, struct PCB *pcb)
{
    char operacion[5];
    char registro[5];
    int valor = 0;
    char registro_fuente[5];

    if (sscanf(instruccion, "%s %s %s", operacion, registro, registro_fuente) < 1)
    {
        mvprintw(15, 2, "                                                      ");
        mvprintw(15, 2, "espacio vacio ");
        refresh();
        return 4; // sicnifica que es un espacio que no se cuenta
    }

    strUpper(operacion);
    strUpper(registro);
    strUpper(registro_fuente);

    if (strcmp(operacion, "END") == 0)
    {
        mvprintw(14, 2, "                                                      ");
        mvprintw(14, 2, "se encontro un end ");
        refresh();
        return 1;
    }

    if (strcmp(operacion, "MOV") == 0)
    {
        if (isdigit(registro_fuente[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro_fuente);
        }
        else if (strcmp(registro_fuente, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro_fuente, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro_fuente, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro_fuente, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro_fuente);
            refresh();
            return 1;
        }

        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX = valor;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX = valor;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX = valor;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX = valor;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else if (strcmp(operacion, "ADD") == 0)
    {
        if (isdigit(registro_fuente[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro_fuente);
        }
        else if (strcmp(registro_fuente, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro_fuente, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro_fuente, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro_fuente, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro_fuente);
            refresh();
            return 1;
        }
        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                             ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX += valor;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX += valor;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX += valor;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX += valor;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else if (strcmp(operacion, "SUB") == 0)
    {
        if (isdigit(registro_fuente[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro_fuente);
        }
        else if (strcmp(registro_fuente, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro_fuente, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro_fuente, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro_fuente, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro_fuente);
            refresh();
            return 1;
        }
        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                                ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX -= valor;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX -= valor;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX -= valor;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX -= valor;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else if (strcmp(operacion, "MUL") == 0)
    {
        if (isdigit(registro_fuente[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro_fuente);
        }
        else if (strcmp(registro_fuente, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro_fuente, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro_fuente, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro_fuente, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro_fuente);
            refresh();
            return 1;
        }
        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                         ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX *= valor;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX *= valor;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX *= valor;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX *= valor;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else if (strcmp(operacion, "DIV") == 0)
    {
        if (isdigit(registro_fuente[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro_fuente);
        }
        else if (strcmp(registro_fuente, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro_fuente, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro_fuente, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro_fuente, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro_fuente);
            refresh();
            return 1;
        }
        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                    ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (valor == 0)
        {
            mvprintw(14, 2, "                                            ");
            mvprintw(14, 2, "divicion por cero no valido ");
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX = pcb->AX / valor;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX = pcb->BX / valor;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX = pcb->CX / valor;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX = pcb->DX / valor;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else if (strcmp(operacion, "DEC") == 0)
    {

        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                             ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX--;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX--;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX--;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX--;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        } ////////////////////////////
    }
    else if (strcmp(operacion, "JNZ") == 0)
    {
        if (isdigit(registro[0]))
        {
            // Si el registro fuente es un número, convertirlo a entero
            valor = atoi(registro);
        }

        if (strcmp(registro, "AX") == 0)
        {
            valor = pcb->AX;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            valor = pcb->BX;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            valor = pcb->CX;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            valor = pcb->DX;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        
        if (valor < 0)
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "el valor es menor a cero %d", valor);
            refresh();
            return 1;
        }

        if (valor == 0)
        {
            mvprintw(18, 2, "desbo %d",valor);
            refresh();
            return 4;
        }
        
        if (valor >= pcb->marcos*4-1)
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(18, 2, "desbordamiento %d",pcb->marcos*4-1);
            refresh();
            return 1;
        }

        pcb->PC_real = valor - 1;

        
    }
    ////////////////
    else if (strcmp(operacion, "INC") == 0)
    {
        if (isdigit(registro[0]))
        {
            mvprintw(14, 2, "                                             ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }

        if (strcmp(registro, "AX") == 0)
        {
            pcb->AX++;
        }
        else if (strcmp(registro, "BX") == 0)
        {
            pcb->BX++;
        }
        else if (strcmp(registro, "CX") == 0)
        {
            pcb->CX++;
        }
        else if (strcmp(registro, "DX") == 0)
        {
            pcb->DX++;
        }
        else
        {
            mvprintw(14, 2, "                                                      ");
            mvprintw(14, 2, "registro no valido %s", registro);
            refresh();
            return 1;
        }
    }
    else
    {
        mvprintw(14, 2, "                                                ");
        mvprintw(14, 2, "operacion no valida %s", operacion);
        refresh();
        return 1;
    }
    return 0;
}

void ver_swap(int long posicion)
{
    int long copia = posicion;
    int long contador = posicion + 3; // Cambiar de +15 a +3
    int aux2 = 1;
    char *linea = NULL;

    mvprintw(37, 32, "-------SWAP---------------------------------------------------------");
    while (aux2 <= 5) // Puedes ajustar este valor si deseas menos bloques
    {
        int aux = 38;
        int long hexa = posicion;
        posicion = copia;
        contador = posicion + 3; // Mostrar 4 líneas por bloque

        while (posicion <= contador)
        {
            char *linea = leer_linea_cadena(posicion * 32);
            int pos = 12 + (15 * aux2);
            if (linea)
            {
                mvprintw(aux, pos, "                   ");
                mvprintw(aux, pos, "%lx-> %s ", hexa, linea);
                mvprintw(0, 2, "$> ");
            }
            else if (hexa >= 65536)
            {
                mvprintw(aux, pos, "XXX->--------------- ");
                mvprintw(0, 2, "$> ");
            }
            else
            {
                mvprintw(aux, pos, "%ld->///////////// ", hexa);
                mvprintw(0, 2, "$> ");
            }
            posicion++;
            hexa++;
            aux++;
        }

        aux2++;
        copia += 4; // Avanzar 4 posiciones en lugar de 16
    }

    refresh();
}

void ver_ram()
{
    mvprintw(37, 110, "------ Contenido de RAM ------"); // Título

    int pos_y = 38;  // Fila inicial para mostrar el contenido
    int pos_x = 110; // Columna inicial

    int inicio = ver_ram_con * 16; // 4 bloques * 4 líneas por bloque = 16 líneas

    for (int i = 0; i < 16; i++) // Mostrar 16 líneas de RAM
    {
        int linea = inicio + i;

        mvprintw(pos_y + i, pos_x, "%02d %03d:", i / 4 + ver_ram_con * 4, i); // Mostrar índice

        mvprintw(pos_y + i, pos_x + 15, "|");

        for (int j = 0; j < 32; j++) // Mostrar 32 caracteres por línea
        {
            char c = Ram[linea][j]; // Acceder a la RAM: línea i completa

            if (c >= 32 && c <= 126)
                mvprintw(pos_y + i, pos_x + 17 + j, "%c", c); // Caracter imprimible
            else
                mvprintw(pos_y + i, pos_x + 17 + j, "."); // No imprimible
        }

        mvprintw(pos_y + i, pos_x + 49, "|"); // Cierre de línea
        mvprintw(0, 2, "$> ");
    }
}
/* 
int verificar_fallo_pagina(int pid, int pagina)
{
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        // VERIFICA SI EN LA RAM ESTA LA PAGINA QUE OCUP
        if (ram_arr[i].id == pid && pagina == ram_arr[i].num_marco)
        {
            return 0; // No hay fallo de página
        }
    }
    return 1; // Hay fallo de página
}
*/

int verificar_fallo_pagina(int pid, int pagina)
{
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].lleno == 1 &&
            ram_arr[i].id == pid &&
            ram_arr[i].num_marco == pagina)
        {
            return 0; // NO hay fallo
        }
    }

    return 1; // Sí hay fallo
}

int encontrar_marco_libre_ram()
{
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].lleno == 0)
        {
            return i; // Devuelve el índice del primer marco libre
        }
    }
    return -1; // No hay marcos libres
}



 
int leer_linea_archivoBin(struct PCB *pcb, struct PCB **lista)
{
    int end = 0;
    char *cadena = NULL;
    int MARCO;
    int pid_proseso;
    int marco_de_la_ram;
    int base;
    int offset = pcb->PC_real % TAMANO_MARCOS;
    int fila;
    MARCO = obtenerMarco(pcb->pid, pcb->PC_real / TAMANO_MARCOS);
    pid_proseso = pcb->pid;
    long linea = arreglo[MARCO].pri + pcb->PC_real % TAMANO_MARCOS; // liena de donde lee el archivo  ,tambien la voy a usar para leer hacer el fallo de pag

    actualizar_uso_si_toca(MARCO);

    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].num_marco == MARCO)
        {
            ram_arr[i].bit_retraso = 1;
        }
    }

    actualizar_uso_si_toca(MARCO);

    if (verificar_fallo_pagina(pid_proseso, MARCO) != 0)
    {
        fallos_de_pagina++;
        copiar_marco_completo_swap_a_ram(pid_proseso, MARCO, linea);
    }

    marco_de_la_ram = obtener_marco_en_ram(pcb->pid, MARCO);

    if (ram_arr[marco_de_la_ram].hospedaje > 4)
    {
        for (int i = 0; i <= TAMANO_RAM; i++)
        {
            if (ram_arr[i].id == pcb->pid)
            {
                if (ram_arr[i].num_marco == MARCO)
                {
                    if (ram_arr[i].tipo == 1)
                    {
                        ram_arr[i].tipo = 2; // acerto aciende a t2
                    }
                }
            }
        }
    }
    else
        ram_arr[marco_de_la_ram].hospedaje++;

    base = ram_arr[marco_de_la_ram].pri;
    fila = base + offset;
    cadena = leer_linea_ram(fila);
    // cadena = leer_linea_cadena(linea * 32);
    strcpy(pcb->IR, cadena);
    end = procesar_instruccion(cadena, pcb);

    if (pcb->PC >= 351)
    {
        return 0;
    }

    pcb->PC_real++;

    if (end == 4)
    {
        return 1;
    }
    pcb->PC++;
    pcb->KCPU += 12;
    usu[pcb->UID] += 12;
    pcb->KCPUxU = usu[pcb->UID];
    actu_U(lista, pcb->UID);

    if (end == 1 || end == 3)
        return 0;
    return 1;
}


int obtener_marco_en_ram(int pid, int MARCO)
{
    for (int i = 0; i <= TAMANO_RAM; i++)
    {
        if (ram_arr[i].id == pid)
        {
            if (ram_arr[i].num_marco == MARCO)
            {
                return i;
            }
        }
    }
    return -1;
}

// Retorna 0 si todas las líneas fueron copiadas correctamente
int copiar_marco_completo_swap_a_ram(int pid, int marco_swap, long linea_Swap)
{
    // Verificar si el marco en swap es válido
    if (marco_swap < 0 || marco_swap >= 4096 || arreglo[marco_swap].pri == -1)
    {
        return -1; // Marco inválido
    }

    // Encontrar un marco libre en la RAM
    int marco_ram = encontrar_marco_libre_ram();
    if (marco_ram == -1)
    {
        // planificador_reloj(); // Llamar al planificador FIFO si no hay espacio
        A_Remplazo_P[A_Replazo_pagina]();

        // algoridmo_CAR();

        marco_ram = encontrar_marco_libre_ram();
        if (marco_ram == -1)
        {
            return -1; // No hay espacio en RAM
        }
    }

    int posicion_ram = marco_ram * TAMANO_MARCOS; // La posición de inicio del marco en la RAM
    long posicion_inicial_swap = arreglo[marco_swap].pri * 32;
    char buffer[32];
    char *copia;

    for (int lin = 0; lin < TAMANO_MARCOS; lin++)
    {
        copia = leer_linea_cadenaXD((lin * 32) + posicion_inicial_swap);
        if (copia == NULL)
        {
            continue; // Si la línea no existe, pasar a la siguiente
        }
        strncpy(buffer, copia, 32);
        buffer[32] = '\0';
        strncpy(Ram[posicion_ram + lin], buffer, 32);
    }

    ram_arr[marco_ram].bit_retraso = 1;
    ram_arr[marco_ram].uso = 0;
    ram_arr[marco_ram].lleno = 1;
    ram_arr[marco_ram].id = pid;
    ram_arr[marco_ram].num_marco = marco_swap;
    ram_arr[marco_ram].referencia = 1;
    ram_arr[marco_ram].hospedaje = 0;
    ram_arr[marco_ram].accesos = 0;

    int esta_lista = verificar_historial(pid, marco_swap);

    if (esta_lista == 0)
    {
        ram_arr[marco_ram].tipo = 1; // T1
    }
    else if (esta_lista == 1)
    {
        eliminar_de_B1(pid, marco_swap);
        ram_arr[marco_ram].tipo = 2; // T2
        if (p < TAMANO_RAM - 1)
            p++;
    }
    else
    {
        eliminar_de_B2(pid, marco_swap);
        ram_arr[marco_ram].tipo = 2; // T2
        if (p > 0)
            p--;
    }

    return 0;
}

int meter_b1_b2(int id, int marco)
{
    int tipo = -1;

    // Buscar el marco en RAM para saber su tipo
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].id == id && ram_arr[i].num_marco == marco && ram_arr[i].lleno)
        {
            tipo = ram_arr[i].tipo;
            break;
        }
    }

    if (tipo == 1)
    {
        agregar_a_B1(id, marco); // Va a B1 si viene de T1
        return 1;
    }
    else if (tipo == 2)
    {
        agregar_a_B2(id, marco); // Va a B2 si viene de T2
        return 2;
    }

    return -1; // No se encontró el marco en RAM o no tiene tipo válido
}

int verificar_historial(int id, int marco)
{
    for (int i = 0; i < tam_b1; i++)
    {
        if (B1[i].id == id && B1[i].num_marco == marco)
            return 1;
    }

    for (int i = 0; i < tam_b2; i++)
    {
        if (B2[i].id == id && B2[i].num_marco == marco)
            return 2;
    }

    return 0;
}

void eliminar_de_B1(int id, int num_marco)
{
    int i;
    for (i = 0; i < tam_b1; i++)
    {
        if (B1[i].id == id && B1[i].num_marco == num_marco)
        {
            for (int j = i; j < tam_b1 - 1; j++)
            {
                B1[j] = B1[j + 1];
            }
            tam_b1--;
            break;
        }
    }
}

void eliminar_de_B2(int id, int num_marco)
{
    int i;
    for (i = 0; i < tam_b2; i++)
    {
        if (B2[i].id == id && B2[i].num_marco == num_marco)
        {
            for (int j = i; j < tam_b2 - 1; j++)
            {
                B2[j] = B2[j + 1];
            }
            tam_b2--;
            break;
        }
    }
}

char *leer_linea_cadenaXD(long int posicion)
{
    if (salida != NULL)
    {
        fseek(salida, posicion, SEEK_SET);

        char *buffer = malloc(TAMANIO_LINEA);
        if (buffer == NULL)
            return NULL;

        int i = 0;
        while (fread(&buffer[i], sizeof(char), 1, salida) > 0)
        {
            if (buffer[i] == '\n' || buffer[i] == '\0')
            {
                buffer[i] = '\0';
                return buffer;
            }
            i++;
        }

        if (i == 0)
        {
            free(buffer);
            return NULL;
        }

        buffer[i] = '\0';
        return buffer;
    }
    return NULL;
}

char *leer_linea_ram(int fila)
{
    if (fila < 0 || fila >= 64)
        return NULL;

    char *buffer = malloc(32); // 32 chars + \0
    if (buffer == NULL)
        return NULL;

    for (int i = 0; i < 31; i++)
    {
        char c = Ram[fila][i];
        if (c == '\0' || c == '\n')
        {
            buffer[i] = '\0';
            return buffer;
        }
        buffer[i] = c;
    }

    buffer[32] = '\0';
    return buffer;
}

///////////////////////////////reloj

int planificador_reloj()
{
    while (1)
    {
        if (ram_arr[puntero_reloj].bit_retraso == 0)
        {
            // Se encontró una víctima, devolvemos el índice actual
            break;
        }

        // Si el bit está en 1, se pone a 0 y se avanza
        ram_arr[puntero_reloj].bit_retraso = 0;
        puntero_reloj = (puntero_reloj + 1) % TAMANO_RAM; // Circular
        mostrar_cap_ram();
        usleep(100000);
    }

    ram_arr[puntero_reloj].id = 0;
    ram_arr[puntero_reloj].lleno = 0;
    ram_arr[puntero_reloj].bit_retraso = 0;

    // Avanzamos el puntero global después de insertar
    puntero_reloj = (puntero_reloj + 1) % TAMANO_RAM;
}
void mostrar_cap_ram()
{
    mvprintw(37, 165, "--RAM: Estado de Marcos  reloj -"); // Título

    int pos_y = 38;
    int pos_x = 165;

    // Recorrer cada marco en la RAM
    for (int j = 0; j < TAMANO_RAM; j++)
    {
        if (puntero_reloj == j)
        {
            mvprintw(pos_y + j, pos_x - 4, "->");
        }
        else
        {
            mvprintw(pos_y + j, pos_x - 4, "  ");
        }

        if (ram_arr[j].lleno)
        {
            // Marco lleno: mostrar PID y página
            mvprintw(pos_y + j, pos_x, "                                 ");
            mvprintw(pos_y + j, pos_x, "- %d: PID %d N-marco %d puntero %d", j + 1, ram_arr[j].id, ram_arr[j].num_marco, ram_arr[j].bit_retraso);
        }
        else
        {
            // Marco vacío
            mvprintw(pos_y + j, pos_x, "- %d: Libre                      ", j + 1);
        }
    }

    mvprintw(0, 2, "$> "); // Restaurar el prompt
    refresh();             // Actualizar la pantalla
}
/////////////////
int planificador_fifo()
{
    static int cuenta = 0; // Variable estática para llevar el índice del marco a reemplazar

    // Reiniciar el marco actual en la RAM
    ram_arr[cuenta].lleno = 0; // Marcar el marco como libre
    ram_arr[cuenta].id = 0;    // Reiniciar el ID del proceso

    // Mover al siguiente marco (comportamiento circular)
    cuenta++;
    if (cuenta == TAMANO_RAM)
    {
        cuenta = 0; // Reiniciar el contador si alcanza el número de marcos
    }
    return cuenta;
}

void mostrar_cap_ram_fifo()
{
    mvprintw(37, 165, "--RAM: Estado de Marcos fifo -"); // Título

    int pos_y = 38;  // Fila inicial para mostrar los marcos
    int pos_x = 165; // Columna inicial

    // Recorrer cada marco en la RAM
    for (int j = 0; j < TAMANO_RAM; j++)
    {
        mvprintw(pos_y + j, pos_x - 4, "  ");

        if (ram_arr[j].lleno)
        {
            // Marco lleno: mostrar PID y página
            mvprintw(pos_y + j, pos_x, "                                 ");
            mvprintw(pos_y + j, pos_x, "- %d: PID %d N-marco %d ", j + 1, ram_arr[j].id, ram_arr[j].num_marco);
        }
        else
        {
            // Marco vacío
            mvprintw(pos_y + j, pos_x, "- %d: Libre                      ", j + 1);
        }
    }

    mvprintw(0, 2, "$> "); // Restaurar el prompt
    refresh();             // Actualizar la pantalla
}
///////////////lru

int planificador_lru()
{
    int mayor_indice = 0;
    int mayor = ram_arr[0].uso;
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].uso > mayor)
        {
            mayor = ram_arr[i].uso;
            mayor_indice = i;
        }
    }

    /*
    mvprintw(16, 2, "                                                      ");
    mvprintw(16, 2, "mayor indice %d", mayor_indice);
    refresh();
    */

    ram_arr[mayor_indice].lleno = 0;
    ram_arr[mayor_indice].id = 0;
    return mayor_indice;
}

void actualizar_uso_si_toca(int marco_actual)
{
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].lleno == 1)
        {
            ram_arr[i].uso++; // tiempo sin usarse aumenta
        }

        if (ram_arr[i].num_marco == marco_actual && ram_arr[i].lleno == 1)
        {
            ram_arr[i].uso = 0; // este marco fue accedido
        }
    }
    // mostrar_cap_ram_lru();
}
void mostrar_cap_ram_lru()
{
    mvprintw(37, 165, "--RAM: Estado de Marcos lru -"); // Título

    int pos_y = 38;  // Fila inicial para mostrar los marcos
    int pos_x = 165; // Columna inicial

    // Recorrer cada marco en la RAM
    for (int j = 0; j < TAMANO_RAM; j++)
    {
        mvprintw(pos_y + j, pos_x - 4, "  ");
        if (ram_arr[j].lleno)
        {
            // Marco lleno: mostrar PID y página
            mvprintw(pos_y + j, pos_x, "                                 ");
            mvprintw(pos_y + j, pos_x, "- %d: PID %d N-marco %d uso %d", j, ram_arr[j].id, ram_arr[j].num_marco, ram_arr[j].uso);
        }
        else
        {
            // Marco vacío
            mvprintw(pos_y + j, pos_x, "- %d: Libre                      ", j);
        }
    }

    mvprintw(0, 2, "$> "); // Restaurar el prompt
    refresh();             // Actualizar la pantalla
}
/////////////////////////////////////////////////////////////car

void mostrar_estado_car()
{
    int pos_x = 0;
    int pos_y = 35;
    int linea = pos_y;

    // Título
    mvprintw(linea++, pos_x, "-- Estado de CAR --");
    mvprintw(linea++, pos_x, "Puntero clock t1 : %d  t2- %d     ", puntero_clock_t1, puntero_clock_t2);
    mvprintw(linea++, pos_x, "Parámetro p: %d", p);
    mvprintw(linea++, pos_x, "-- RAM (T1/T2) --");

    // Mostrar marcos de RAM
    for (int j = 0; j < TAMANO_RAM; j++)
    {
        if (ram_arr[j].lleno)
        {
            const char *lista = (ram_arr[j].tipo == 1) ? "T1" : (ram_arr[j].tipo == 2) ? "T2"
                                                                                       : "??";

            mvprintw(linea, pos_x, "                                           ");
            mvprintw(linea++, pos_x, "- %2d: PID %d Marco %d R=%d [%s]", j, ram_arr[j].id, ram_arr[j].num_marco, ram_arr[j].referencia, lista);
        }
        else
        {
            mvprintw(linea++, pos_x, "                                                   ");
            mvprintw(linea, pos_x, "- %2d: Libre", j);
        }
    }

    linea = 35;
    int columna = 35;
    mvprintw(linea++, columna, "-- Lista B1 --");
    for (int i = 0; i < TAMANO_B1_B2; i++)
    {
        mvprintw(linea, columna, "                       ");
        if (i < tam_b1)
            mvprintw(linea++, columna, "- [%d] ID %d Marco %d", i, B1[i].id, B1[i].num_marco);
        else
            mvprintw(linea++, columna, "- [%d] (vacío)", i);
    }
    linea++;

    mvprintw(linea++, columna, "-- Lista B2 --");
    for (int i = 0; i < TAMANO_B1_B2; i++)
    {
        mvprintw(linea, columna, "                         ");
        if (i < tam_b2)
            mvprintw(linea++, columna, "- [%d] ID %d Marco %d", i, B2[i].id, B2[i].num_marco);
        else
            mvprintw(linea++, columna, "- [%d] (vacío)", i);
    }

    mvprintw(0, 2, "$> ");
    refresh();
}

int algoridmo_CAR()
{
    int encontrado = 0;
    int tamano_t1 = contar_T1();
    int lista_a_cambiar = 1; // para t1 1 para t2 2

    if (tamano_t1 > 0 && tamano_t1 > p)
    {
        while (1)
        {
            if (ram_arr[puntero_clock_t1].tipo == 1)
            { // Solo T1
                if (ram_arr[puntero_clock_t1].referencia == 0)
                {
                    // Guardar datos antes de borrar
                    int id_victima = ram_arr[puntero_clock_t1].id;
                    int marco_victima = ram_arr[puntero_clock_t1].num_marco;

                    // Limpiar el marco
                    ram_arr[puntero_clock_t1].id = 0;
                    ram_arr[puntero_clock_t1].lleno = 0;
                    ram_arr[puntero_clock_t1].referencia = 0;
                    ram_arr[puntero_clock_t1].tipo = 0;

                    // Agregar a B1
                    agregar_a_B1(id_victima, marco_victima);

                    // Avanzar el reloj
                    puntero_clock_t1 = (puntero_clock_t1 + 1) % TAMANO_RAM;
                    break;
                }
                else
                {
                    // Segunda oportunidad
                    ram_arr[puntero_clock_t1].referencia = 0;
                }
            }
            // Avanzar el puntero de reloj
            puntero_clock_t1 = (puntero_clock_t1 + 1) % TAMANO_RAM;
        }
    }
    else
    {
        while (1)
        {
            if (ram_arr[puntero_clock_t2].tipo == 2)
            { // Solo T2
                if (ram_arr[puntero_clock_t2].referencia == 0)
                {
                    // Guardar datos antes de borrar
                    int id_victima = ram_arr[puntero_clock_t2].id;
                    int marco_victima = ram_arr[puntero_clock_t2].num_marco;

                    // Limpiar el marco
                    ram_arr[puntero_clock_t2].id = 0;
                    ram_arr[puntero_clock_t2].lleno = 0;
                    ram_arr[puntero_clock_t2].referencia = 0;
                    ram_arr[puntero_clock_t2].tipo = 0;

                    // Agregar a B2
                    agregar_a_B2(id_victima, marco_victima);

                    // Avanzar el reloj
                    puntero_clock_t2 = (puntero_clock_t2 + 1) % TAMANO_RAM;
                    break;
                }
                else
                {
                    // Segunda oportunidad
                    ram_arr[puntero_clock_t2].referencia = 0;
                }
            }
            // Avanzar el puntero de reloj
            puntero_clock_t2 = (puntero_clock_t2 + 1) % TAMANO_RAM;
        }
    }
}





int contar_T1()
{
    int contador = 0;
    for (int i = 0; i < TAMANO_RAM; i++)
    {
        if (ram_arr[i].lleno && ram_arr[i].tipo == 1)
        {
            contador++;
        }
    }
    return contador;
}

void agregar_a_B1(int id, int marco)
{
    // Si está llena, eliminar el más antiguo (FIFO)
    if (tam_b1 == TAMANO_B1_B2)
    {
        for (int i = 1; i < tam_b1; i++)
        {
            B1[i - 1] = B1[i];
        }
        tam_b1--;
    }

    B1[tam_b1].id = id;
    B1[tam_b1].num_marco = marco;
    tam_b1++;
}

void agregar_a_B2(int id, int marco)
{
    // Si está llena, eliminar el más antiguo (FIFO)
    if (tam_b2 == TAMANO_B1_B2)
    {
        for (int i = 1; i < tam_b2; i++)
        {
            B2[i - 1] = B2[i];
        }
        tam_b2--;
    }
    B2[tam_b2].id = id;
    B2[tam_b2].num_marco = marco;
    tam_b2++;
}




///////////////////////////////////////////////////////////////esto es para la creacion de multiples prosesos con comandos

int crear_proceso_con_archivo(char *b, struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS)
{
    mvprintw(13, 2, "                                                         ");
    mvprintw(13, 2, "se selecciono el archivo %s ", b);
    refresh();

    FILE *archivo = fopen(b, "r");
    if (!archivo)
    {
        mvprintw(13, 2, "Error al abrir archivo %s", b);
        refresh();
        return 0;
    }

    struct PCB *PCB = crear_pcb();
    strcpy(PCB->fileName, b);
    strcpy(PCB->IR, "----------");
    PCB->programa = archivo;
    PCB->KCPUxU = usu[PCB->UID];
    PCB->marcos = bloques(archivo);

    int parentesco = buscarProgramaPorNombre(*lista, *pcb, PCB);
    int resultado = 1;

    if (false)
    { // Puedes cambiar esta condición si más adelante habilitas la detección de familia
        PCB->familia = parentesco;
    }
    else
    {
        resultado = escribirParteEnArchivoBinario(PCB->programa, PCB->pid);
    }

    if (resultado == 0)
    {
        insertar_PCB(terminados, PCB);
        mvprintw(13, 2, "                                                ");
        mvprintw(13, 2, "archivo es muy grande %s ", b);
        refresh();
        return 0;
    }
    else if (resultado == 41)
    {
        insertar_PCB(LISTOS, PCB);
        return 1;
    }

    insertar_PCB(lista, PCB);
    Dispacher(lista);
    return 1;
}

/* 
void crea_multiples_p(struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS)
{
    char *archivo = "c";
    for (int i = 0; i < 3; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }
    archivo = "b";

    for (int i = 0; i < 3; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }

    archivo = "a";
    for (int i = 0; i < 3; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }
}
*/

void crea_multiples_p(struct PCB **lista, struct PCB **pcb, struct PCB **terminados, struct PCB **LISTOS)
{
    
    char *archivo = "c";
    /* 
    for (int i = 0; i < 10; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }
    archivo = "b";

    for (int i = 0; i < 10; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }

    archivo = "a";
    for (int i = 0; i < 10; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }
    */
    //archivo = "a";
    for (int i = 0; i < 7; i++)
    {
        crear_proceso_con_archivo(archivo, lista, pcb, LISTOS, terminados);
    }
}







void guardar_datos(int a, int b)
{
    FILE *archivo = fopen("datos.txt", "a"); // Abrir en modo append
    if (archivo == NULL)
    {
        perror("No se pudo abrir el archivo");
        return;
    }

    switch (A_Replazo_pagina)
    {
    case 0:
        fprintf(archivo, "Algoritmo: Reloj\n");
        break;
    case 1:
        fprintf(archivo, "Algoritmo: FIFO\n");
        break;
    case 2:
        fprintf(archivo, "Algoritmo: LRU\n");
        break;
    case 3:
        fprintf(archivo, "Algoritmo: CAR\n");
        break;
    }
    fprintf(archivo, "Fallos de página: %d,accesos a memoria %d \n", a,b);
    fclose(archivo);
}
////////////////////////////////////////////////////////////////
